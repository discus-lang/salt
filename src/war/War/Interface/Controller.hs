...
module War.Interface.Controller
        ( Config        (..)
        , controller)
where
import War.Driver.Base
import BuildBox.Control.Gang
import War.Interface.VT100      as VT100
...
controller config gang chainsTotal chanResult
 ...
 where
        ...
        -- We've got input on the console, wait for already running
        -- tests then bail out.
        go_input jobResults
         ...
         -> if empty'
             then do
                ...
                if gangState == GangFinished
                 ...
                 -- Just because the gang loop has finished doesn't mean that
                 -- all the workers have. Wait for the workers to finish,
                 -- then read the remaining bits off the queue.
                 then do
                        ...
                        -- TODO: This might be due to a bug in the gang code.
