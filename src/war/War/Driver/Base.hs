...
instance Pretty Job where
 ppr (Job jobId actionName spec _)
         = string "Job"
         % (string $ show jobId) % string actionName % string (show spec)


-------------------------------------------------------------------------------
-- | A chain of jobs to run one after another.
--   Jobs later in the list are dependent on earlier ones,
--   so if a job fails then we skip the rest.
data Chain = Chain [Job]
...
--   This is the information that the interactive interface needs to
